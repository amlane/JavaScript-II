...
//using .map() to solve same problem as a stretch challenge
let fullNameWithMap = runners.map(x => x.first_name + x.last_name);
console.log(fullNameWithMap);
...
const greatestDonations = runners.filter(runner => runner.donation > 200);
console.log(greatestDonations);

// Problem 2: Return a list of company names all uppercase
const companyList = runners.map(x => x.company_name.toUpperCase());
